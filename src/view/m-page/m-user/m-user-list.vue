	...
	<div class="userListStyle">
		...
		<Table :loading="userTableLoading" size="small" border :columns="userColumns" :data="userListData">
			...
			<template slot-scope="{ row, index }" slot="action">
				<Button type="primary" size="small" icon="ios-create-outline" @click="editorUser(row)" style="margin-right: 8px;"></Button>
				<Button type="success" icon="ios-key" size="small" @click="resetPassword(row)"></Button>
			</template>
		</Table>
		...
		<Modal v-model="showUserEditor" title="编辑用户信息" footer-hide>
			...
			<user-form v-if="showUserEditor" :user-id="userId">{{$t('change')}}</user-form>
		</Modal>
...
<script>
	...
	import UserForm from '@/view/page/user-management/components/user-form.vue'
...
<style>
	@media screen and (min-width:300px) and (max-width:900px) {
		...
		.userListStyle .ivu-modal-body {
			padding: 0;
		}

		.userListStyle .ivu-table-cell {
			padding: 0;
			margin: 0;
		}
	}
